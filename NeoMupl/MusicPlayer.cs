...
namespace NeoMupl
{
    ...
    public class MusicPlayerNew : MusicPlayer
    {
        private NeoMuplDLL.Player player = new NeoMuplDLL.Player();
        private MusicData myData = null;
        private NeoMuplDLL.MusicInfo info = new NeoMuplDLL.MusicInfo();

        public MusicPlayerNew(IntPtr handle)
        {
            player.Initialize(handle);
        }

        private NeoMuplDLL.MusicPlayerType PlayMethod2MusicPlayerType(PlayMethod method)
        {
            switch (method)
            {
                case PlayMethod.DirectShow:
                    return NeoMuplDLL.MusicPlayerType.DirectShow;
                case PlayMethod.DirectMusic:
                    return NeoMuplDLL.MusicPlayerType.DirectMusic;
                case PlayMethod.MCI:
                    return NeoMuplDLL.MusicPlayerType.MCI;
                default:
                    throw new ArgumentException();
            }
        }

        #region MusicPlayer メンバ
        ...
        public MusicData Data
        {
            ...
            {
                myData = value;
                info.FileName = myData.FileName;
                info.LoopStart = myData.LoopStart;
                info.LoopEnd = myData.LoopEnd;
                info.Vomume = myData.Volume;
                info.MIDIPort = "";
                info.type = PlayMethod2MusicPlayerType(myData.PlayMethod);
            }
        }

        public bool Loop
        {
            get { return player.IsLoop(); }
        }
        ...
        public double Volume
        {
            get { return info.Vomume; }
        }

        public IEnumerable<string> GetDirectMusicPorts()
        {
            return player.GetMIDIPorts();
        }
